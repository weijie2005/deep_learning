...
二、激活函数(Activation Function)
        ...
        5.常用的激活函数：
            1.sigmoid 函数：将输入映射到 (0, 1) 之间，常用在二分类问题中。
               1. 公式：f(x) = 1 / (1 + exp(-x))
               2. 范围：(0, 1)
               3. 导数：f'(x) = f(x) * (1 - f(x))
            ...
            3. ReLU 函数：将输入大于 0 的部分直接输出，小于等于 0 的部分输出 0，常用在隐藏层中。                
                1. 公式：f(x) = max(0, x)
                2. 范围：[0, ∞)
